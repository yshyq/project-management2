...
| 权限 | RBAC + 数据范围 + 凭证类型授权 |
| 部署 | Nginx + Vue 静态资源 + FastAPI 服务 + PostgreSQL + MinIO，建议 Docker Compose 起步 |
...
- 上传组件统一调用 `/api/files/upload`，业务表单只保存返回的文件 ID。
...
- API 前缀统一为 `/api`，版本前缀为 `/api/v1`。
- SQLAlchemy 管理 PostgreSQL 数据模型。
- Alembic 管理数据库迁移。
- Pydantic schema 区分 `Create`、`Update`、`Read`。
- MinIO 文件对象 Key 不直接暴露真实业务路径，下载统一走鉴权接口。
- 凭证解密只允许在服务层完成，并必须记录审计日志。
...
### 6.9 credential_authorization 凭证授权表
...
| GET | `/api/projects/{id}/authorized-credentials` | 已授权凭证 |
...
| POST | `/api/support-tickets/{id}/handle` | 处理支持单 |
| POST | `/api/support-tickets/{id}/transfer` | 转办 |
| POST | `/api/support-tickets/{id}/close` | 关闭 |
...
Nginx
  /            -> frontend dist
  /api         -> backend service
  /uploads     -> file service or backend download API
